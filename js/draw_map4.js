...
function displayCenter( canvas_id, size_x, size_y, message) {
...
function drawLocationBar( canvas_id, canvas_x_start, canvas_x, canvas_y_start, canvas_y, loc_start, loc_end, flipY )
{
    var canvas = $( '#' + canvas_id )[0];
    var ctx = canvas.getContext("2d");
...
{
    ctx.fillStyle = "rgba(67, 28, 93, 0.2)";
    if (index % 2 == 1)
        ctx.fillStyle = "rgba(63, 176, 172, 0.2)";

    ctx.fillRect( get_g_to_x_coord(tad_start, canvas_id), y_start, get_g_to_x_coord(tad_end, canvas_id)-get_g_to_x_coord(tad_start, canvas_id), 8);
}

function get_x_to_g_coord( x, canvas_id )
{
    var n = canvas_id.substr(canvas_id.indexOf('_')+1);
    return parseInt($("#start"+n).val()) + parseInt( Math.abs( $("#end"+n).val() - $("#start"+n).val() ) *  x / 686 );
}

function get_g_to_x_coord( g, canvas_id )
{
    var n = canvas_id.substr(canvas_id.indexOf('_')+1);
    return ( 686 - 0 ) * ( ( g - $("#start"+n).val() )/($("#end"+n).val() - $("#start"+n).val()) );
}
...
function drawHeatMap( canvas_id, canvas_x_start, canvas_x, canvas_y_start, canvas_y, flipY ) 
{
    ...
    if (flipY)
        drawLocationBar( canvas_id, canvas_x_start, canvas_x, 20, canvas_y, $('#start' + p).val(), $('#end' + p).val(), flipY );
    else
        drawLocationBar( canvas_id, canvas_x_start, canvas_x, canvas_y, canvas_y, $('#start' + p).val(), $('#end' + p).val(), flipY );
...
*/
    ...
    for (p = 1; p <= 2; p++)
    {
        if (p % 2 == 1)
        {
            drawHeatMap('heatMapCanvas_'+p, 0, 686, 0, 343, false);
            drawTAD('heatMapCanvas_'+p, 353);
        }
        else
        {
            drawHeatMap('heatMapCanvas_'+p, 0, 686, 20, 363, true);
            drawTAD('heatMapCanvas_'+p, 0);
        }
    }
});
...
function get_x_to_g_coord( x, p )
{
    return parseInt($("#start"+p).val()) + parseInt( Math.abs( $("#end"+p).val() - $("#start"+p).val() ) *  x / 686 );
}
...
$(".navigator").dblclick(function(e) {
    ...
    */
    ...
    if (start_x >= 0 && endin_x <= canvas_endin_x && y <= canvas_image_y)
    {
        //$( "#start" ).empty();
        //$( "#end"   ).empty();
        $( '#start' ).val( get_x_to_g_coord(start_x, 1) );
        $( '#end'   ).val( get_x_to_g_coord(endin_x, 1) );
        //var endVal = get_x_to_g_coord(endin_x, 1);
        //$( '#end'   ).val( endVal );
        //document.getElementsByName('end')[0].value = endVal;

        $( '#s2' ).val( get_x_to_g_coord(start_x, 2) );
        $( '#e2' ).val( get_x_to_g_coord(endin_x, 2) );
