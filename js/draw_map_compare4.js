...
function drawGradientBar( canvas_id, x_start, x_size, y_start, y_size, flipY )
{
    var canvas = $( '#'+ canvas_id )[0];
    var ctx = canvas.getContext("2d");
    drawGradientBar_on_ctx( canvas_id, ctx, x_start, x_size, y_start, y_size, flipY );
}

function drawGradientBar_on_ctx( canvas_id, ctx, x_start, x_size, y_start, y_size, flipY )
{
    ...
    if ( $( '#' + canvas_id ).data("positive") && $( '#' + canvas_id ).data("negative") )
    {
        ...
        //ctx.fillText(String.fromCharCode("8804") + " " + $( '#' + canvas_id ).data("negMin"), x_start+x_size+15, y_start+y_size);
        ctx.fillText($( '#' + canvas_id ).data("posMax"), x_start+x_size+15, y_start + 5    );
        ctx.fillText($( '#' + canvas_id ).data("negMin"), x_start+x_size+15, y_start+y_size);

        if ( $( '#' + canvas_id ).data("negMax") != $( '#' + canvas_id ).data("posMin") )
        {
            ctx.fillText( $( '#' + canvas_id ).data("negMax") + " - " + $( '#' + canvas_id ).data("posMin"), x_start+x_size+15, y_start+(y_size-20)/2+10 );
        }
        else
        {
            ctx.fillText( $( '#' + canvas_id ).data("negMax"), x_start+x_size+15, y_start+(y_size-20)/2+10 );
        }
    }
    else
    {
        if ( $( '#' + canvas_id ).data("positive") )
        {
            ...
            //else
            {
                ...
                ctx.fillText($( '#' + canvas_id ).data("posMax"), x_start+x_size+15, y_start       +5 );
                ctx.fillText($( '#' + canvas_id ).data("posMin"), x_start+x_size+15, y_start+y_size+5);
            }
        }
        else if ( $( '#' + canvas_id ).data("negative") )
        {
            ...
            //else
            {
                ...
                ctx.fillText($( '#' + canvas_id ).data("negMax"), x_start+x_size+15, y_start       +5);
                ctx.fillText($( '#' + canvas_id ).data("negMin"), x_start+x_size+15, y_start+y_size+5);
            }
        }
        else
        {
...
function drawLocationBar( canvas_id, canvas_x_start, canvas_x, canvas_y_start, canvas_y, loc_start, loc_end, chrom, flipY )
{
    var canvas = $( '#' + canvas_id )[0];
    var ctx = canvas.getContext("2d");
    drawLocationBar_on_ctx( canvas_id, ctx, canvas_x_start, canvas_x, canvas_y_start, canvas_y, loc_start, loc_end, chrom, flipY );
}

function drawLocationBar_on_ctx( canvas_id, ctx, canvas_x_start, canvas_x, canvas_y_start, canvas_y, loc_start, loc_end, chrom, flipY )
{
    ...
    if (flipY)
    {
        ...
        ctx.fillText(chrom, 0, canvas_y_start);
    }
    else
    {
        ctx.fillText(chrom, 0, canvas_y_start+10);
    }
}
...
function drawTAD( canvas_id, y_start )
{
    ...
    drawTAD_on_ctx( canvas_id, ctx, y_start, canvasWidth );
}

function drawTAD_on_ctx( canvas_id, ctx, y_start, length )
{
    ...
    {
        draw_rect( ctx, tad_arr[i][0], tad_arr[i][1], i, y_start, canvas_id, length );
    }
}

function draw_rect( ctx, tad_start, tad_end, index, y_start, canvas_id, length )
{
    ctx.fillStyle = "rgba(235, 189, 91, 0.6)";
    if (index % 2 == 1)
        ctx.fillStyle = "rgba(45, 84, 139, 0.6)";

    ctx.fillRect( get_g_to_x_coord(canvas_id, tad_start, length), y_start, get_g_to_x_coord(canvas_id, tad_end, length)-get_g_to_x_coord(canvas_id, tad_start, length), 10);
}
...
function drawHeatMap( canvas_id, canvas_x_start, canvas_x, canvas_y_start, canvas_y, flipY )
{
    ...
    drawHeatMap_on_ctx( canvas_id, ctx, canvas_x_start, canvas_x, canvas_y_start, canvas_y, flipY );
}

function drawHeatMap_on_ctx( canvas_id, ctx, canvas_x_start, canvas_x, canvas_y_start, canvas_y, flipY ) 
{
    ...
    color3_hex = '#ccccff'; // light blue for slider range
    $( '#sliderRangeNeg_' + p ).css( 'background', 'rgba(' + hexToRgb(color2_hex).r + ',' + hexToRgb(color2_hex).g + ',' + hexToRgb(color2_hex).b + ', 1.0)' );  // background blue
    $( '#sliderRangeNegAppendDiv_' + p).css( 'background', 'rgba(255, 255, 255, 1.0)' ); // positive values white
    $( '#sliderRangeNeg_' + p + ' .ui-slider-range' ).css( 'background', 'rgba(' + hexToRgb(color3_hex).r + ',' + hexToRgb(color3_hex).g + ',' + hexToRgb(color3_hex).b + ', 1.0)' );  // slider-range light blue

    var ctx2 = $( '#' + canvas_id )[0].getContext("2d");
    if (ctx == ctx2)
    ...
    //Draw gradient bar
    if (flipY)
        drawGradientBar_on_ctx(canvas_id, ctx, 10, 20, (canvas_y + canvas_y_start)/2 - 10, (canvas_y-canvas_y_start)/2, true);
    else
        drawGradientBar_on_ctx(canvas_id, ctx, 10, 20,         10, canvas_y/2-10, false);
    ...
    for (i = num_rows; i > 0; i--)
    {
        ...
        for (j = 0; j < i; j++)
        {
            ...
            if ( $( '#' + canvas_id ).data("negative") )
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        rbg_str = getRgbaStr( hexToRgb(color2_hex).r, hexToRgb(color2_hex).g, hexToRgb(color2_hex).b, (1.0 * ($( '#' + canvas_id ).data("negMax") - val))/( $( '#' + canvas_id ).data("negMax") - $( '#' + canvas_id ).data("negMin") ) );
                    }
                }
            }
            ...
            //{
                ...
                {
                    drawSquare(ctx, cur_x, cur_y, size_x-size_x/1000, size_y-size_y/1000, rbg_str);
                    //else
                    //{
    ...
    if (flipY)
        drawLocationBar_on_ctx( canvas_id, ctx, canvas_x_start, canvas_x, canvas_y_start, canvas_y, $('#start' + p).val(), $('#end' + p).val(), $('#chr' + p).val(), flipY );
    else {
	if ( p==3 ) {p=1;}
        drawLocationBar_on_ctx( canvas_id, ctx, canvas_x_start, canvas_x, canvas_y, canvas_y, $('#start' + p).val(), $('#end' + p).val(), $('#chr' + p).val(), flipY );
	p = 1;
	}
    if ( p == 1 )
...
*/
    ...
    //$( '#sliderRangePosAppendDiv_' + p ).css( 'background', 'rgba(' + hexToRgb(color1_hex).r + ',' + hexToRgb(color1_hex).g + ',' + hexToRgb(color1_hex).b + ', 1.0)' );
    $( '#sliderRangePos_' + p + ' .ui-slider-range' ).css( 'background', 'rgba(' + hexToRgb(color1_hex).r + ',' + hexToRgb(color1_hex).g + ',' + hexToRgb(color1_hex).b + ', 0.5)' );
    $( '#sliderRangeNeg_' + p + ' .ui-slider-range' ).css( 'background', 'rgba(' + hexToRgb(color3_hex).r + ',' + hexToRgb(color3_hex).g + ',' + hexToRgb(color3_hex).b + ', 1.0)' );
    ...
    //drawHeatMap('heatMapCanvas_'+3, 0, canvasWidth, 0, canvasHeight, false);
});
...
// when slide the any of the first two sliders, update the first two sliders. Fan 031318
// when slde the slider of the diff matrix, update the diff matrix accordingly
                for (p = 1; p <= 2; p++)
                {
                    if (p % 2 == 1)
                    {
	...
	var sliderRangePosID = 'sliderRangePos_' + n;
        ...
        $( '#' + spinnerNegMaxID ).spinner({
            ...
            spin: function( event, ui ) {
                ...
                if ( $('#' + canvas_id ).data("positive") && ui.value >= $( '#' + spinnerPosMinID ).spinner( "value") )
                {
                    return false;
                }          
            },
        ...
        $( '#' + sliderRangeNegID ).slider({
            ...
            slide: function (event, ui) {
                if ( $( '#' + canvas_id ).data("positive") && ui.values[1] >= $( '#' + sliderRangePosID ).slider( "values", 0 ) )
                {
                    return false;
                }
                $('#sliderRangeNegAppendDiv_' + n).css('width', 100 - ui.values[1] +'%');
            ...
            change: function (event, ui) {
                ...
                drawHeatMap('heatMapCanvas_'+n, 0, canvasWidth, 0, canvasHeight, false);
            }

        }).append('<div class="sliderRangeNegAppendDiv" id="sliderRangeNegAppendDiv_' + n + '" style="width: '+ (100 - $( '#' + sliderRangeNegID ).slider( "values", 1 )) +'%"></div>');
...
{
    var n = canvas_id.substr(canvas_id.indexOf('_')+1);
    var canvas = $( '#navigator_1' )[0]; // draw navigator for the first matrix
    var danvas = $( '#navigator_2' )[0]; // draw navigator for the second matrix
    var eanvas = $( '#navigator_3' )[0]; // draw navigator (shaded triangle) for diff matrix Fan 031218, navigator and embedCanvas3 can be found in compare-test1.php
    if (! canvas )
    {
        return;
    }
    ...
    etx.fillStyle = "rgba(150, 150, 150, 0.35)";
    ...
    var rxt1 = $( '#embedCanvas3_1' )[0];
    var rxt2 = $( '#embedCanvas3_2' )[0];
    ...
    if (rxt1)
        rtx1 = rxt1.getContext("2d");
    if (rxt2)
        rtx2 = rxt2.getContext("2d");
    ...
    if (rxt1)
        rtx1.clearRect(canvas_start_x,canvas_start_y,canvas_endin_x-canvas_start_x,canvas_endin_y-canvas_start_y);
    if (rxt2)
        rtx2.clearRect(canvas_start_x,canvas_start_y,canvas_endin_x-canvas_start_x,canvas_endin_y-canvas_start_y);
    ...
    if (start_x >= 0 && endin_x <= canvas_endin_x && y <= canvas_image_y)
    {
        ...
        etx.beginPath();
        etx.moveTo(start_x, canvas_endin_y);
        etx.lineTo(start_x, canvas_image_y);
        etx.lineTo(start_x + offst_x, canvas_image_y - start_y);
        etx.lineTo(endin_x, canvas_image_y);
        etx.lineTo(endin_x, canvas_endin_y);
	...
	etx.fill();
        ...
        if (rxt1)
        {
            rtx1.fillStyle = "rgba(50, 50, 50, 0.2)";
            rtx1.fillRect(start_x, 0, size_x, canvas_endin_y);
            rtx1.fillRect(endin_x - size_x , 0, size_x, canvas_endin_y);
        }

        if (rxt2)
        {
            rtx2.fillStyle = "rgba(50, 50, 50, 0.2)";
            rtx2.fillRect(start_x, 0, size_x, canvas_endin_y);
            rtx2.fillRect(endin_x - size_x , 0, size_x, canvas_endin_y);
        }
    }
}
