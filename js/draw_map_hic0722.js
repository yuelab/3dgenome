...
function clearCanvas( canvas_id )
{
    ...
    clearCanvas_on_ctx( canvas_id, ctx, 0, canvas_width, 0, canvas_height+20);
}

function clearCanvas_on_ctx( canvas_id, ctx, canvas_x_start, canvas_x, canvas_y_start, canvas_y )
{
    ctx.fillStyle = "rgba(255,255,255,1)"; //white
    ctx.fillRect(canvas_x_start,canvas_y_start,canvas_x,canvas_y); //clear canvas region
}

function drawGradientBar( canvas_id, ctx, x_start, x_size, y_start, y_size )
{
...
//        ctx.fillText(String.fromCharCode("8804") + " " + $( canvas_id ).data("negMin"), x_start+x_size+15, y_start+y_size-10);
        ...
        ctx.fillText($( canvas_id ).data("negMin"), x_start+x_size+15, y_start+y_size-10);
    ...
    else
    {
        if ( $( canvas_id ).data("positive") )
        {
            ...
            ctx.fillText($( canvas_id ).data("posMax"), x_start+x_size+15, y_start       -5 );
            ctx.fillText($( canvas_id ).data("posMin"), x_start+x_size+15, y_start+y_size-10);
        }
...
{
    ctx.fillStyle = "black";
    var text = cur_x_tick; //Number(cur_x_tick).toLocaleString(); //cur_x_tick.toString();
...
function drawLocationBar( canvas_id, ctx, canvas_x_start, canvas_x, canvas_y_start, canvas_y, loc_start, loc_end )
{
...
function drawLegend( ctx, canvas_x_start, canvas_x, canvas_y_start, canvas_y )
{
    ...
    if ( $(":input[name=browser]:checked").val() == "none" )
    {
    ctx.fillStyle = "black";
...
function drawHeatMap( canvas_id )
{
    var canvas = $( canvas_id )[0];
    if (! canvas )
    {
    ...
    ctx =  $( '#heatMapCanvas' )[0].getContext("2d");

    if (! ctx)
    {
        return;
    }
    drawHeatMap_on_ctx(canvas_id, ctx, 0, canvas_width, 0, canvas_height );
}

function drawHeatMap_on_ctx( canvas_id, ctx, canvas_x_start, canvas_x, canvas_y_start, canvas_y ) 
{
    ...
    clearCanvas_on_ctx( canvas_id, ctx, canvas_x_start, canvas_x, canvas_y_start, canvas_y+20 );
    ...
    //Draw gradient bar
    drawGradientBar(canvas_id, ctx, 10, 10, 20, canvas_y/2-10);
        ...
        for (j = 0; j < i; j++)
        {
            ...
            if ( $( canvas_id ).data("positive") )
            {
                ...
                else
                {
                    ...
                    {
                        rbg_str = getRBGAStr( hexToRgb(color1_hex).r, hexToRgb(color1_hex).g, hexToRgb(color1_hex).b, (1.0 * ( val - $( canvas_id ).data("posMin") ))/( $( canvas_id ).data("posMax") - $( canvas_id ).data("posMin") ) );
                    }
                }
            }
            ...
            if ( $( canvas_id ).data("negative") )
            {
                ...
                else
                {
                    ...
                    {
                        rbg_str = getRBGAStr( hexToRgb(color2_hex).r, hexToRgb(color2_hex).g, hexToRgb(color2_hex).b, (1.0 * ( val - $( canvas_id ).data("negMin") ))/( $( canvas_id ).data("negMax") - $( canvas_id ).data("negMin") ) );
                    }
                }
            }
            ...
            //{
                ...
                {
                    drawTriangle(ctx, cur_x, cur_y, size_x-0.00001, size_y-0.00001, rbg_str);
                }
                else
                {
                    //if (j < num_rows-1)
                    {
                       drawSquare(ctx, cur_x, cur_y, size_x-0.00001, size_y-0.00001, rbg_str);
                    }
                }
            //}
            cur_x += size_x/2;
    ...
    drawLocationBar( canvas_id, ctx, canvas_x_start, canvas_x, canvas_y, canvas_y+20, $('#start').val(), $('#end').val() );
...
function drawNavigator( canvas_id, x, y )
{
    ...
    if (extend2)
    {
        ...
        etx2.clearRect(canvas_start_x,0,canvas_endin_x-canvas_start_x,10);
    }
...
//        ctx.stroke();
        ...
        if (extend2)
        {
            ...
            etx2.fillRect(start_x, 0, size_x, 10);
            etx2.fillRect(endin_x - size_x , 0, size_x, 10);
        }
    }
}
...
function drawTAD( canvas_id )
{
    ...
    drawTADctx( canvas_id, ctx, 0 );
}

function drawTADctx( canvas_id, ctx, start_y )
{
    tad_arr = $( '#' + canvas_id ).data("tad");
    for (i = 0; i < tad_arr.length; i++)
    {
        draw_rect( ctx, tad_arr[i][0], tad_arr[i][1], i, start_y);
    }
}

function draw_rect( ctx, tad_start, tad_end, index, start_y )
{
    ctx.fillStyle = "rgba(235, 189, 91, 0.6)";
    if (index % 2 == 1)
        ctx.fillStyle = "rgba(45, 84, 139, 0.6)";

    ctx.fillRect( get_g_to_x_coord(tad_start), start_y, get_g_to_x_coord(tad_end)-get_g_to_x_coord(tad_start), 10);
}
...
$("#navigator").mousemove(function(e) {
    ...
    if (mouseDownFlag && Math.abs(x - mouseDownX) > 5)
    {
        ...
        drawHeatMap_on_ctx( '#heatMapCanvas', $( '#heatMapCanvas' )[0].getContext("2d"), 0 + x - mouseDownX, canvas_width + x - mouseDownX, 0, canvas_height);
    }
...
$("#navigator").mouseleave(function(e) {
    if ( mouseDownFlag )
    {
        mouseDownFlag = false;
        clearCanvas( '#heatMapCanvas' );
        drawHeatMap_on_ctx( '#heatMapCanvas', $( '#heatMapCanvas' )[0].getContext("2d"), 0, canvas_width, 0, canvas_height);
    }
});
