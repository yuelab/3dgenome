...
function plot4c (id_name, tissue)
{
    var ctx = $( '#' + id_name)[0].getContext("2d");
    if (!ctx) return;
    plot4c_on_ctx( id_name, ctx, tissue );
}

function plot4c_on_ctx( id_name, ctx, tissue )
{
    ...
    var y_list = $( "#" + id_name ).data("tissue_to_y")[tissue];
...
////////y-axis label
    ...
    //ctx.fillText(y_label, plot_start_x+(plot_endin_y-plot_start_y)/2, plot_total_length+32);
    ctx.restore();
...
//////////////////////////////////////
    ...
    for (i = 0; i <= y_tick_num; i++)
    {
        ...
        //draw number labels
        ctx.fillStyle = "rgba(0,0,0,1)";
        text = "" + Math.round(y_min + y_range/y_tick_num * i * 10)/10;
        if (i < y_tick_num)
...
/////////////////////////////////////////
    ...
    ctx.linewidth = 5;
    ctx.strokeStyle = getRGBAStr( hexToRgb( rgbColor ).r, hexToRgb( rgbColor ).g, hexToRgb( rgbColor ).b, 0.75);
...
function drawIndicator(canvas_id, x, y)
{
    ...
    var currentColor = hexToRgb($('#colorInput' ).val());
    //draw indicator ball
    ctx.strokeStyle = getRGBAStr(currentColor.r, currentColor.g, currentColor.b);
    ctx.beginPath();
    ...
    ctx.fillStyle = getRGBAStr(currentColor.r, currentColor.g, currentColor.b, 0.25);

    var g1 = Math.floor($( "#" + plot_id ).data("x")[index] / res) * res;
    //var x1 = ( g1 - start ) / ( end - start )  * (plot_endin_x - plot_start_x) + plot_start_x;
    var g2 = (Math.floor($( "#" + plot_id ).data("x")[index] / res)+1) * res;
    //var x2 = ( g2 - start ) / ( end - start )  * (plot_endin_x - plot_start_x) + plot_start_x; 
    var x1 = x_pos_list[index];
    var x2 = x_pos_list[index+1];
    ...
    if (dtx)
    {
        ...
        dtx.fillStyle = getRGBAStr(currentColor.r, currentColor.g, currentColor.b, 0.25);
    ...
    $('.chiaPlot').each(function() {
        if (this.id.split('_')[1] == canvas_id.split('_')[1])
        {
            ...
            if (rtx)
            {
                ...
                rtx.fillStyle = getRGBAStr(currentColor.r, currentColor.g, currentColor.b, 0.25);
...
function drawHighlight(canvas_id, x1, x2)
{
    ...
    var xn1 = x_pos_list[index1];
    var xn2 = x_pos_list[index2];
...
function retrieveHighlight(canvas_id, mouseLeftFlag)
{
    ...
    plot4c( canvas_id, 'GM12878' );
    plot4c( canvas_id, 'HMEC' );
    plot4c( canvas_id, 'HUVEC' );
    plot4c( canvas_id, 'IMR90' );
    plot4c( canvas_id, 'K562' );
    plot4c( canvas_id, 'KBM7' );
    plot4c( canvas_id, 'NHEK' );
    ...
    embed = $( '#' + ucsc_cov_id)[0];
    if (embed)
        etx = embed.getContext("2d");
    if (etx)
    {
        clearCanvas( ucsc_cov_id );
        etx.fillStyle = getRGBAStr(currentColor.r, currentColor.g, currentColor.b, 0.25);
    }
    ...
    for (var i = 0; i < hi_array.length; i++)
    {
        if ( hi_array[i] == 1)
        {
            ...
            for (var j = 0; j < $( '#' + dhs_id ).data('color_list').length; j++)
            {
                var arc_x1 = $("#" + dhs_id).data("x1_list")[j];
                var arc_x2 = $("#" + dhs_id).data("x2_list")[j];

                if ( (arc_x1 >= x1 && arc_x1 < x2) || (arc_x2 >= x1 && arc_x2 < x2) )
                {
                    dhs_list.push(j);
                }
            }
    ...
    if ( highlightNum <= 1 || contiFlag )
    {
        for (var i = 0; i < dhs_list.length; i++)
        {
            j = dhs_list[i];
            var arc_x1 = $("#" + dhs_id).data("x1_list")[j];
            var arc_x2 = $("#" + dhs_id).data("x2_list")[j];
            color = $("#" + dhs_id).data("color_list")[j];
            alpha = $("#" + dhs_id).data("alpha_list")[j];
            drawLinkageSingle(dhs_id, arc_x1, arc_x2, color, alpha);
        }
    }
    else
    {
        var duplicates = dhs_list.reduce(function(acc, el, i, arr) {
  if (arr.indexOf(el) !== i && acc.indexOf(el) < 0) acc.push(el); return acc;
    ...
    for (var i = 0; i < hi_array.length; i++)
    {
        if ( hi_array[i] == 1)
        {
            ...
            if (etx)
            {
                etx.fillRect(x1, 0, x2-x1, $( '#' + ucsc_cov_id)[0].height );                
            }
            if (dtx)
            {
                dtx.fillRect(x1, 0, x2-x1, $( '#' + dhs_id)[0].height );
            }
