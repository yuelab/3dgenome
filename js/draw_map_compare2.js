...
function drawXLabel( ctx, cur_x, cur_y, cur_x_tick, canvas_x, flipY )
{
    ctx.fillStyle = "black";
...
function drawLocationBar( canvas_id, canvas_x_start, canvas_x, canvas_y_start, canvas_y, loc_start, loc_end, chrom, flipY )
{
    ...
    while (cur_x_tick < loc_end )
    {
        ...
        drawXLabel( ctx, cur_x, canvas_y_start, cur_x_tick, canvas_x, flipY);
    ...
    if (flipY)
        ctx.fillText(chrom, 0, 7);
    else
        ctx.fillText(chrom, 0, canvas_y+20);
...
/*
    if ( $( '#' + canvas_id ).data("negative") )
    {
        ...
        $( '#sliderRangeNeg' ).slider({
            ...
            c
    console.log( 'here' );hange: function (event, ui) {
                if ( $( canvas_id ).data("positive") && ui.values[1] >= $( '#sliderRangePos' ).slider( "values", 0 ) )
                {
                    return false;
                }
...
function quantileNorm( array1, array2 )
{
    var sorted_array1 = array1.slice().sort(function (a, b) { return parseFloat(a) - parseFloat(b);});
    var sorted_array2 = array2.slice().sort(function (a, b) { return parseFloat(a) - parseFloat(b);});

    var n = 0;
    var rank_array1 = [];
    for (var i = 0; i < array1.length; i++)
    {
        var index = sorted_array1.indexOf( array1[i] ) + n;
        rank_array1.push(index);
    }

    console.log( array1 );
    console.log( rank_array1 );

    n = 0;
    var rank_array2 = [];
    for (var i = 0; i < array2.length; i++)
    {
        var index = sorted_array2.indexOf( array2[i] ) + n;
        rank_array2.push(index);
    }

    var average_array = [];
    for (var i = 0; i < array1.length; i++)
    {
        average_array.push( ( sorted_array1[i] + sorted_array2[i] )/2 );
    }

    console.log( average_array );

    var quan_array1 = Array.apply(null, Array(array1.length)).map(Number.prototype.valueOf,0);
    for (var i = 0; i < array1.length; i++)
    {
        quan_array1[ i ] = average_array[ rank_array1[i] ];
    }    

    var quan_array2 = Array.apply(null, Array(array2.length)).map(Number.prototype.valueOf,0);

    for (var i = 0; i < array2.length; i++)
    {
        quan_array2[ i ] = average_array[ rank_array2[i] ];
    }

    var diff_array = []
    for (var i = 0; i < array1.length; i++)
    {
        diff_array.push( ( quan_array1[i] - quan_array2[i] ) );
    }

    $('#diffCanvas').data('matrix', diff_array );
    $('#diffCanvas').data('num_rows', $('#heatMapCanvas_1').data('num_rows'));
    
    console.log( diff_array );

    console.log( $('#diffCanvas').data() );
    $('#diffCanvas').data('positive', 1);
    $('#diffCanvas').data('negative', 1);
    $('#diffCanvas').data('negMax', -50);
    $('#diffCanvas').data('negMin', -100);
    $('#diffCanvas').data('posMax', 100);
    $('#diffCanvas').data('posMin', 50);

    drawHeatMap( 'diffCanvas', 0, 800, 0, 400, false );
}
$("#diffButton").click(function(e) {
    var canvas1 = $('<canvas/>', { id: 'diffCanvas', height: 400, width: 800});
    $('body').append(canvas1);
    ctx = $('#diffCanvas')[0].getContext('2d');
    ctx.canvas.height = 400;
    ctx.canvas.width = 800;

    quantileNorm( $('#heatMapCanvas_1').data('matrix'), $('#heatMapCanvas_2').data('matrix') );
});
