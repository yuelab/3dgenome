...
function drawHeatMap( canvas_id, canvas_x_start, canvas_x, canvas_y_start, canvas_y ) 
{
    ...
    {
        if ( $("#heatMapCanvas").data("resolution") != $('#resolution').val() )
        {
            ctx.fillStyle = "#9900FF";
...
function drawNavigator( canvas_id, x, y )
{
    ...
    var empty = $( '#emptyCanvas' )[0];

    if (empty)
    {
        var mtx = empty.getContext("2d");
        mtx.clearRect(canvas_start_x,canvas_start_y,canvas_endin_x-canvas_start_x,10);
    }
...
//        ctx.stroke();
        ...
        {
            var mtx = empty.getContext("2d");
            mtx.fillStyle = "rgba(50, 50, 50, 0.2)";
            mtx.fillRect(start_x, 0, size_x, canvas_endin_y);
            mtx.fillRect(endin_x - size_x , 0, size_x, canvas_endin_y);
        }
...
function draw_TAD( canvas_id )
{
    ...
    {
        draw_rect( ctx, tad_arr[i][0], tad_arr[i][1], i, 0);
    }
}

function draw_rect( ctx, tad_start, tad_end, index, endin )
{
    ctx.fillStyle = "rgba(67, 28, 93, 0.2)";
    if (index % 2 == 1)
        ctx.fillStyle = "rgba(63, 176, 172, 0.2)";

    ctx.fillRect( get_g_to_x_coord(tad_start), endin, get_g_to_x_coord(tad_end)-get_g_to_x_coord(tad_start), 10);
}
...
{
    return ( 686 - 0 ) * ( ( g - $("#start").val() )/($("#end").val() - $("#start").val()) );
}
